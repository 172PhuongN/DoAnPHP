<?php 
    ...
    class TracnghiemOnlineController{
        ...
        public function listbaithi() {
            ...
            if (isset($_COOKIE['userName'])) {
                ...
                foreach ($listbaithi as $baithi) {
                    $baithi->daLam = $this->TracnghiemOnlneModel->kiemTraDaLamBai($id_nguoidung, $baithi->id_baithi, $id_monhoc);
                }
            } else {
        ...
        public function baithibymonhoc() {
            ...
            $baiThi = $this->TracnghiemOnlneModel->getBaiThiById($id_baithi); 
            $thoiGianThi = $baiThi['time']; // hoặc thời gian thi bạn lưu trong DB
        ...
        public function nopbai() {
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST' && $id_nguoidung) {
                ...
                $id_lambai = $this->TracnghiemOnlneModel->saveBaiLam($id_nguoidung, $id_baithi, $answers,$thoigian_batdau,$id_monhoc);
                // Lấy email, tên người dùng, tên bài thi để gửi mail
                $userInfo =$id_nguoidung;
                
                $baithi = $this->BaithitracnghiemModel->getById($id_baithi);
                $tenBaiThi = $baithi->ten_baithi;
                sendMailAfterSubmit($userInfo, $userInfo, $tenBaiThi);
