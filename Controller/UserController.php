    ...
    class UserController{
        ...
        public function editUser(){
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["updateUser"])) {
                ...
                if ($this->userModel->updateUser($userName, $password, $roleId, $isDeleted, $currentUser->email, $currentUser->image, $userName)) {
        ...
        public function profile() {
            ...
            $subjectScores = $this->userModel->getUserAverageScoreBySubject($userName); // Lấy điểm trung bình theo môn
        ...
        public function editProfile() {
            ...
            include "./View/User/editProfile.php";
        }
        public function updateProfile() {
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["updateProfile"])) {
                ...
                // Lấy userName ban đầu từ cookie
                $originalUserName = $_COOKIE['userName'];
                $newUserName = $_POST["userName"];
                $password = $_POST["password"];
                $email = $_POST["email"];
                
                // Lấy thông tin user hiện tại bằng userName ban đầu
                $currentUser = $this->userModel->getUserbyuserName($originalUserName);
                ...
                $image = $currentUser->image; // Giữ hình ảnh cũ nếu không upload hình mới
                if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
                    $uploadDir = 'images/users/'; // Thư mục lưu hình ảnh
                    ...
                    if (move_uploaded_file($_FILES['image']['tmp_name'], $uploadPath)) {
                        $image = $uploadPath; // Lưu đường dẫn hình ảnh
                    } else {
                        $errorMessage = "Có lỗi xảy ra khi upload hình ảnh!";
                        include "./View/User/editProfile.php";
                        return;
                    }
                }
                ...
                if ($this->userModel->updateUser($newUserName, $password, $currentUser->roleId, $currentUser->isDeleted, $email, $image, $originalUserName)) {
                    // Cập nhật cookie nếu userName thay đổi
                    setcookie("userName", $newUserName, time() + (86400 * 30), "/");
                    header("Location: index.php?controller=user&action=profile&message=success");
                    exit();} else {
                        $errorMessage = "Có lỗi xảy ra khi cập nhật!";
                        include "./View/User/editProfile.php";
                    }
